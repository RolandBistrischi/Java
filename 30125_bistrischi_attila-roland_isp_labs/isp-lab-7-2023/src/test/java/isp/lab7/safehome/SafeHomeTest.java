...
public class SafeHomeTest {
    ...
    public void instantiateDoorController() {
        ...
        /*Tenant tenant = new Tenant("Marius");
        Map<Tenant, AccessKey> tenantAccessKeyMap = new HashMap<>();
        tenantAccessKeyMap.put(tenant, new AccessKey("qwerty", tenant));
        Door door = new Door(DoorStatus.CLOSE, tenant);
        AccessLog accessLog = new AccessLog(tenant.getName(), LocalDateTime.now(), "OPEN", DoorStatus.CLOSE, "eroare");*/

        ctrl =new DoorLockController(new HashMap<>());

        //DoorLockController doorLockController = new DoorLockController(tenantAccessKeyMap, door, accessLog);
